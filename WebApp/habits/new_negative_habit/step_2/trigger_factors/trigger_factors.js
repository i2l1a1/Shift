...
send_page_name_to_server("new_negative_habit/step_1/trigger_factors/trigger_factors.html").then(r => {

});

const accept_button = document.querySelector(".accept_button_div");

const url_for_check = `http://127.0.0.1:9091/stage_2/get_unlock_status_stage_2/${localStorage.getItem("active_habit")}`;
...
get_data_from_server(url_for_check).then((data_from_server) => {
    ...
    if (data_from_server === 1) {
        ...
        window.location.href = "../trigger_factors_test/trigger_factors_test.html";
...
accept_button.addEventListener("click", () => {
    ...
    const url = `http://127.0.0.1:9091/edit_negative_habit/stage_2/start_trigger_tracking/${localStorage.getItem("active_habit")}`;
    let data_for_send = {
        "number_of_days": 5
    }
